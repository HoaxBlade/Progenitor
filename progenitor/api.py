...
def enhance(
    ...
    *,
    ...
    if max_speed:
        # Defaults are set after loading the model — we detect architecture first
        pass
    ...
    # 0. Block removal for CNNs (remove entire residual blocks)
    if is_conv_heavy and opts.max_speed:
        try:
            from progenitor.optimizations.block_removal import apply_block_removal
            apply_block_removal(model, 0.75)
            applied_passes.append("block removal 75%")
        except Exception as e:
